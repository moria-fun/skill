...
import { getAssociatedTokenAddressSync, getAccount, TokenAccountNotFoundError, TokenInvalidAccountOwnerError, createAssociatedTokenAccountIdempotentInstruction, NATIVE_MINT, createCloseAccountInstruction, TOKEN_PROGRAM_ID, createSyncNativeInstruction, ASSOCIATED_TOKEN_PROGRAM_ID } from '@solana/spl-token';
...
// src/constants.ts
var CLAWLAB_PROGRAM_ID = new PublicKey(idl_default.address);
...
async function getAccountData(accountAddress, accountType, program, commitment) {
  const address = accountAddress instanceof PublicKey ? accountAddress : new PublicKey(accountAddress);
...
function deriveUserTokenAccount(baseMint, user) {
  const [userOutputTokenAccount] = PublicKey.findProgramAddressSync(
...
var CreatorService = class extends ClawlabProgram {
  ...
  async ClaimFeeTx(params) {
    ...
    return new Transaction().add(...preInstructions, createPoolIx, ...postInstructions);
  }
};
var PoolService = class extends ClawlabProgram {
  ...
  async CreateTx(params) {
    ...
    return new Transaction().add(createPoolIx);
  }
  ...
  async MintToTx(params) {
    ...
    createQuoteMintAtaIx && preInstructions.push(createQuoteMintAtaIx);
    const amountIn = params.copies.mul(new BN(2e8));
    ...
    const unwrapSolIx = unwrapSOLInstruction(payer, payer);
    unwrapSolIx && postInstructions.push(unwrapSolIx);
    return new Transaction().add(...preInstructions, mintToIx, ...postInstructions);
  }
  ...
  async RefundTx(params) {
    ...
    const unwrapSolIx = unwrapSOLInstruction(payer, payer);
    unwrapSolIx && postInstructions.push(unwrapSolIx);
    return new Transaction().add(...preInstructions, refundIx, ...postInstructions);
  }
};
var UserService = class extends ClawlabProgram {
  ...
  async ClaimTokenTx(params) {
    ...
    return new Transaction().add(...preInstructions, claimUserTokenIx);
  }
};
...
export { CLAWLAB_PROGRAM_ID, ClawlabClient, idl_default as ClawlabIdl, ClawlabProgram, CreatorService, DAMM_V2_PROGRAM_ID, METAPLEX_PROGRAM_ID, PoolService, StateService, UserService, createClawlabProgram, createDammV2Program, deriveBaseVault, deriveConfig, deriveEventAuthority, deriveMintMetadata, derivePool, derivePoolAuthority, deriveQuoteVault, deriveUserTokenAccount, findAssociatedTokenAddress, getAccountCreationTimestamp, getAccountCreationTimestamps, getAccountData, getOrCreateATAInstruction, getTotalTokenSupply, unwrapSOLInstruction, wrapSOLInstruction };
